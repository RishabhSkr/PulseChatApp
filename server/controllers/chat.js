...
const newGroupChat = TryCatch(async (req, res, next) => {
  ...
  // Emit events to all members of the group chat
  emitEvent(req, ALERT, allMembers, `Welcome to ${name} group chat`);
  // Emit event to all members of the group chat except the creator
  emitEvent(req, REFETCH, members, `New group chat ${name} created`);
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString()) {
    return next(new ErrorHandler('You are not the creator of this chat', 400));
  }
  ...
  emitEvent(
    ...
    ALERT,
    uniqueMembers,
    `You are added to ${chat.name} group chat`
  );
  emitEvent(
    req,
    REFETCH,
...
const removeMember = TryCatch(async (req, res, next) => {
  ...
  if (!chat.members.some(member => member.toString() === userId)) {
    return next(new ErrorHandler('User is not a member of this group', 404));
  }
...
const renameGroup = TryCatch(async (req, res, next) => {
  const chatId = req.params.id;
  console.log('Attempting to renaming chat:', chatId);
  const { name } = req.body;
  if (!name) {
  ...
  emitEvent(req, REFETCH, chat.members, `Group chat name changed to ${name}`);
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, REFETCH, members, `Chat deleted by ${req.user}`);
