...
import { NEW_REQUEST, REFETCH } from '../constants/constants.js';
...
const searchUser = TryCatch(async (req, res,next) => {
  ...
  // all users from my chats i.e. all users with whom i have chatted freinds
  const allUsersFromMyChats = myChats.map(chat => chat.members).flat();
  // finding all users except me and my friends
  const allUsersExcemptMeAndMyFriends = await User.find(
    {
      _id: {$nin: allUsersFromMyChats}, // not in my chats
      name: { $regex: name, $options: 'i' },
    }
  )
  
  const users = allUsersExcemptMeAndMyFriends.map(({_id,name,avatar})=>({
    _id,
    name,
    avatar: avatar.url
  })
